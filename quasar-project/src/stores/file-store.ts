...
export const useFileStore = defineStore('file', {
    ...
    getters: {
		...
		getActiveFile: (state) => {
			return state.localFiles.find((file) => file.id === state.activeFileId);
		},

		getContentById: (state) => (fileId: string, elementId: string, index: any) => {
            // console.log('getContentById', fileId, elementId, index)
            const file = state.localFiles.find((file) => file.id === fileId);
            if (file) {
                if (index !== undefined) {
                    const element = file.data.find((element) => element.elementId === elementId + '_' + index);
                    //console.log('element:', element);
                    if (element) {
                        return element.data.content;
                    }
                } else {
                    const element = file.data.find((element) => element.elementId === elementId);
                    //console.log('element:', element);
                    if (element) {
                        return element.data.content;
                    }
		...
		getElementOfFile: (state) => (fileId: string, elementId: string) => {
			const file = state.localFiles.find((file) => file.id === fileId);
			if (file) {
				return file.data.find((element) => element.elementId === elementId).data.content;
			}
			return null;
		},
    ...
    actions: {
		...
		async getFileById(id: string): Promise<any | null> {
            if (this.localFiles.some((file) => file.id === id) && this.remoteFiles.some((file) => file.id === id)) {
